echo "Ubuntu 22.04 CIS Bencmark Check" > CISOut.txt
echo "===============================" >> CISOut.txt
...
echo "[1.1.2 Configure /tmp]" >> CISOut.txt
#https://stackoverflow.com/questions/11087499/bash-how-do-you-capture-stderr-to-a-variable
{
    outFM=$(findmnt --kernel /tmp 2>&1) 
    echo -e "\n$outFM\n" >> CISOut.txt
    outSysCTl=$(systemctl is-enabled tmp.mount 2>&1)
    echo -e "$outSysCTl\n" >> CISOut.txt
}


echo "[1.1.2.1 Ensure /tmp is a separate partition (Automated)]" >> CISOut.txt
